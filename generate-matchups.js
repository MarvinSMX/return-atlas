#!/usr/bin/env node
/**
 * ETF Matchup Page Generator — ReturnAtlas.com
 * ------------------------------------------------
  ...
  <section class="section"><div class="container">
    <div class="chart-card">
      ...
      <div class="chart-watermark">
        <div class="wm-brand">📈 ReturnAtlas</div>
...
function hubPage(pairs) {
  const year = new Date().getFullYear();
  const updated = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  const title = `ETF Comparisons: ${pairs.length} Head-to-Head Matchups (${year}) | ReturnAtlas`;
